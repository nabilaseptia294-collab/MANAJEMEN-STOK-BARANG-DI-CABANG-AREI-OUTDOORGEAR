...
<div class="layout">
    ...
    <div class="content">
        <h4>Tambah Penerimaan Stok</h4>

        <div class="layout" style="margin-top:10px;gap:20px">
            <div class="card" style="width:60%">
                <p>Informasi Penerimaan</p>
                <input type="hidden" id="penerimaan_id">
                <input type="hidden" id="edit_penerimaan_id">

                <div style="margin-top:20px">
                    <p style="font-size:14px">No. Surat Jalan</p>
                    <input required id="no_surat_jalan" type="text" placeholder="Tuliskan nomor surat jalan" style="width:100%;border:1px solid #000;padding:6px;border-radius:5px">
                </div>

                <div style="margin-top:10px">
                    <p style="font-size:14px">Tanggal Penerimaan</p>
                    <input required id="tanggal_penerimaan" type="date" style="width:100%;border:1px solid #000;padding:6px;border-radius:5px">
                </div>
        ...
        <div style="margin-top:10px;display:flex;justify-content:end;gap:20px">
            <button onclick="simpanPenerimaan()" type="button" style="display: flex; flex-direction: row; gap: 10px; background-color: #E6091A; color: #fff; padding: 10px; align-items: center; box-shadow: 0 4px 8px rgba(0, 0, 0, 0.2); border-radius: 15px; border: none; cursor: pointer; transition: all 0.3s ease;">
    <img src="{{ asset('images/btn-simpan.png') }}" alt="Simpan" style="width: 20px; height: 20px;">
    <span style="font-size: 14px; font-weight: 500;">Simpan Penerimaan</span>
    </button>
...
function loadDetail(id) {
    ...
    <td class="col-aksi">
        <div style="display:flex; gap:10px">
            <div style="cursor:pointer" onclick="openEditModal(
                ${d.id_detail_penerimaan},
                ${d.jumlah_surat_jalan},
                ${d.jumlah_diterima},
                '${d.kondisi.replace(/'/g, "\\'")}',
                ${id}
            )">
            ...
            <div style="cursor:pointer" onclick="hapusDetail(${d.id_detail_penerimaan}, ${id})">
                <img src="{{ asset('images/trash-icon.png') }}" style="width:20px;height:20px">
            </div>
        </div>
    </td>
...
function hapusDetail(detailId, penerimaanId) {
    ...
    .then(data => {
        ...
        loadDetail(penerimaanId);
    })
    .catch(err => {
        console.error(err);
        alert('Gagal menghapus detail. Silakan coba lagi.');
    });
}
...
function openEditModal(id, sj, diterima, kondisi, penerimaanId) {
    document.getElementById('edit_detail_id').value = id;
    document.getElementById('edit_penerimaan_id').value = penerimaanId;
...
function updateDetail() {
    ...
    const penerimaanId = document.getElementById('edit_penerimaan_id').value;
